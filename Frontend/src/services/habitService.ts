import { CreateHabitPayload, HabitUpdate } from "../types/Habit";
import api from "./api";
...
export const editUserHabit = async (token: string, habitId: string, updates: HabitUpdate) => {
  const response = await api.put(`/habtracker/habits/${habitId}`, updates, {
    headers: { Authorization: `Bearer ${token}` },
  });
